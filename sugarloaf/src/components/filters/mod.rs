                ...
                match configured_filter.as_str() {
                    "newpixiecrt" | "fubax_vr" => {
                        ...
                        let builtin_filter = match configured_filter.as_str() {
                            "newpixiecrt" => super::builtin::newpixiecrt,
                            "fubax_vr" => super::builtin::fubaxvr,
                            _ => {
                                continue;
                            }
                        };
...
#[cfg(feature = "filters")]
pub use enabled::FiltersBrush;

/// No-op stub when the `filters` feature is disabled.
...
impl FiltersBrush {
    ...
    pub fn update_filters(
        ...
        if !_filters.is_empty() {
            tracing::warn!(
                "Filters were configured but the `filters` feature is not enabled. \
                 Rebuild with `--features filters` to enable RetroArch shader support."
            );
        }
    }
    ...
    pub fn render(
        ...
        _src_texture: &wgpu::Texture,
        _dst_texture: &wgpu::Texture,
    ) {
        // No-op: filters feature is disabled.
    }
}
