...
pub fn screen(sugarloaf: &mut Sugarloaf, context_dimension: &ContextDimension) {
    ...
    content.sel(title_rt).clear()
        .add_text("Volt Keyboard Shortcuts", FragmentStyle { color: white, ..FragmentStyle::default() })
        .build();
    ...
    let key_style = FragmentStyle { color: key_color, ..FragmentStyle::default() };
    let desc_style = FragmentStyle { color: white, ..FragmentStyle::default() };
    let header_style = FragmentStyle { color: accent, ..FragmentStyle::default() };
    let dim_style = FragmentStyle { color: dim, ..FragmentStyle::default() };
    ...
    // --- TABS ---
    body.new_line().add_text("TABS", header_style).new_line();
    ...
    body.add_text("  Press ", dim_style).add_text("Escape", key_style).add_text(" to close", dim_style);
